(function (global, factory) {
  ...
  class ObjectDetection { 
    ...
    async detect(input$$1) {
      ...
      const [height, width] = getInputTensorDimensions(input$$1);
      ...
      const batched = tidy(() => {
        const imageTensor = fromPixels(input$$1);
  ...
  function getInputTensorDimensions(input$$1) {
    return input$$1 instanceof Tensor ? [input$$1.shape[0], input$$1.shape[1]] : [input$$1.height, input$$1.width];
  }
  ...
  function calculateMaxScores(scores, numBoxes, numClasses) {
    ...
    for (let i = 0; i < numBoxes; i++) {
      let max$$1 = Number.MIN_VALUE;
      let index = -1;
      for (let j = 0; j < numClasses; j++) {
        if (scores[i * numClasses + j] > max$$1) {
          max$$1 = scores[i * numClasses + j];
          index = j;
        }
      }
      maxes[i] = max$$1;
